...
export const fontsDialogPlugin = (editor, opts) => {
  ...
  editor.Commands.add(cmdOpenFonts, {
    ...
    run: (_, sender) => {
      ...
      loadFontList('https://www.googleapis.com/webfonts/v1/webfonts?key=' + opts.api_key).then(
        (fontsList) => {
      ...
      <form class="silex-form grapesjs-fonts">
        <div class="silex-form__group">
          <div class="silex-bar">
            ...
            >
              ${map(
                activeFonts,
                (f) => html` <option value=${f['family']}>${f['family']}</option> `
              )}
            </select>
            <button
              class="silex-button"
              ...
              type="button"
              @click=${() => {
                addFont(editor, config, fonts, activeFonts[fontRef.value.selectedIndex])
        ...
        <div class="silex-form__element">
          ...
          >
            ...
            ${map(fonts, (font) => html` <option value="${font.name}">${font.name}</option> `)}
          </select>
        </div>
        <footer>
          <input
            ...
            value="Cancel"
          />
          <input class="silex-button" type="submit" value="Save" />
        </footer>
      </form>
    `,
    el
